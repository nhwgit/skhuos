...
│   └── makefile
├── common/            # 두 커널이 공유하는 코드 (Type, Paging, Print, String, util)
├── util/
│   └── ImageMaker/    # 부트로더 + 커널 → Disk.img 생성 도구
├── docs/images/       # README 데모 스크린샷
├── run.bat            # 빌드 + QEMU 실행 원클릭 (Windows + WSL)
└── Makefile           # 통합 빌드 (BootLoader → 커널 2종 → ImageMaker → Disk.img)
```
...
Linux 또는 WSL 환경 기준입니다. (Ubuntu: `sudo apt install make nasm gcc gcc-multilib qemu-system-x86 qemu-utils`)

- `make`, `nasm`
- `gcc` — makefile은 Linux 네이티브 gcc/ld/objcopy를 사용합니다. 다른 툴체인을 쓴다면 각 makefile의 `GCC`/`LD`/`OBJCOPY` 변수만 바꾸면 됩니다.
- `qemu-system-x86_64` (실행용)

### 빌드

통합 Makefile은 별도의 빌드 디렉터리에서 실행하도록 작성되어 있습니다.

```bash
mkdir -p build/default && cd build/default
make -f ../../Makefile all      # BootLoader → 커널 2종 → ImageMaker → Disk.img
make -f ../../Makefile clean    # 빌드 산출물 정리
```
...
```bash
qemu-img create -f raw HDD.img 20M                           # 최초 1회
qemu-system-x86_64 -m 64 -hda Disk.img -hdb HDD.img -boot c
```
...
2. **빌드 단계 디스크 이미지 무한 증식 버그**
   - 빌드를 돌리면 `Disk.img`가 끝없이 커져 호스트 디스크 1TB를 가득 채우는 문제를 겪었습니다. 게스트 OS도 아닌 빌드 도구의 사소한 버그가 호스트 자원 전체를 고갈시킬 수 있다는 것을 체감했습니다.
   - 원인은 이미지 생성 도구의 자기참조였습니다. `ImageMaker`는 입력 파일들을 EOF까지 스트리밍 복사(`fread` → `fwrite` 반복)해 이미지 뒤에 이어 붙이는데, 산출물이 입력과 같은 디렉터리에 놓이던 당시 구조에서는 이전 빌드의 이미지가 입력 목록에 섞일 수 있었습니다. 출력 파일을 자기 입력으로 읽는 순간 "쓴 만큼 다시 읽히는" 루프가 되어(`cat A B > B`와 같은 병리) EOF에 도달하지 못한 채 디스크가 찰 때까지 증식합니다.
   - 이미지 도구의 입력을 명시적 경로로 고정하고, 빌드 산출물을 소스 트리 밖 별도 디렉터리(`build/default`)로 분리해 자기참조 가능성을 구조적으로 차단했습니다. 이후 `ImageMaker` 자체에도 입력·출력 동일성 검사(stat 기반)를 추가해 코드 레벨 방어를 완성했습니다. 여기에 파일별 섹터 수 출력처럼 산출물 크기를 기대값과 대조하는 값싼 불변식 검증만 갖춰도 폭주형 버그를 조기에 잡을 수 있다는 교훈을 얻었습니다.
...
5. **수년 만의 전수 리팩토링 — "컴파일이 된다"와 "맞는 코드"의 간극**
   - 졸업 후 코드를 다시 열어 전수 점검하면서, 암시적 함수 선언 탓에 인자 개수가 틀린 호출이 그대로 컴파일되고 있었고, 헤더에 정의된 static 배열이 include하는 파일마다 49KB짜리 사본을 만들고 있었으며, 괄호가 빠진 `if(isOutputBufferFull)`처럼 항상 참인 조건이 잠복해 있었음을 발견했습니다.
   - 헤더를 실제 공개 API와 일치시키고 `-Werror=implicit-function-declaration`을 켜 같은 유형의 문제가 컴파일 단계에서 차단되도록 했으며, 문자열만 다른 프로세스 함수 24벌과 인터럽트 스텁 38벌을 함수 테이블과 NASM 매크로로 통합해 전체 코드를 약 4,300줄에서 3,400줄로 줄였습니다.
   - 관대한 컴파일 설정의 freestanding 환경에서는 "돌아간다"가 정합성을 보증하지 않으며, 경고를 에러로 승격하고 헤더를 단일 진실 공급원으로 유지하는 것이 사후 디버깅보다 훨씬 저렴하다는 것을 배웠습니다.
6. **최신 툴체인으로의 빌드 이전**
   - 제작 당시 Cygwin 크로스 컴파일러를 전제하던 빌드를 WSL 네이티브 gcc로 이전하는 과정에서, 최신 gcc의 기본값(PIE, 스택 프로텍터)이 freestanding 커널의 고정 주소 링크를 깨뜨려 `-fno-pie -fno-stack-protector`를 명시해야 했습니다.
   - 이후 최신 gcc가 구조체 복사를 SSE 명령으로 자동 벡터화하면서, SSE를 활성화하지 않은 커널에서 파일 생성 명령 실행 순간 #UD(invalid opcode) 예외가 발생하는 문제도 겪었습니다. freestanding 커널의 표준 플래그인 `-mno-sse -mno-sse2 -mno-mmx -mno-red-zone`을 명시해 해결했습니다.
   - bare-metal 코드는 툴체인 기본값 변화에 특히 취약하므로, 가정하는 옵션을 빌드 스크립트에 명시적으로 고정하고 빌드부터 QEMU 실행까지의 절차 전체를 `run.bat`으로 저장소에 코드화해 재현성을 확보했습니다.
7. **부팅 방식 전환이 드러낸 드라이버의 잠복 레이스 버그**
   - 부팅을 플로피(CHS)에서 하드디스크 + `INT 13h` 확장 LBA 읽기로 전환하며 검증하던 중, `formatting`이 첫 섹터만 쓰고 무한 대기하고, 파일을 만들어도 `ls`에 보이지 않는 문제를 발견했습니다.
   - 디스크 이미지의 mtime 변화 관찰로 "느린 것"이 아니라 "멈춘 것"임을 판별하고, git의 원본 버전을 별도 빌드해 같은 증상을 재현함으로써 전환 작업의 회귀가 아닌 기존 잠복 버그임을 분리했으며, QEMU의 IDE 트레이스(`--trace ide_sector_write`)로 커널이 발행한 LBA 시퀀스를 추적해 8섹터 쓰기가 3섹터에서 끊기는 것을 확인했습니다.
   - 원인은 둘 다 ATA 프로토콜의 대기 규정 생략이었습니다. ① 명령 발행 전 이전 명령의 BUSY 해제를 확인하지 않아 디바이스가 명령을 무시했고(커널은 오지 않을 DRQ를 영원히 대기) ② 멀티섹터 쓰기에서 첫 DRQ만 확인하고 데이터를 연속으로 밀어넣어 디바이스가 직전 섹터를 플러시하는 동안 보낸 데이터가 유실됐습니다. 각각 BUSY 해제 대기와 "섹터마다 DRQ 대기" 구조로 수정했습니다.
   - 과거 환경에서 정상 동작했던 것은 정확성이 아니라 에뮬레이터의 타이밍이 우연히 관대했던 것입니다. 하드웨어 프로토콜의 대기 조건은 "지금 돌아가니까"로 생략하면 플랫폼이나 버전이 바뀔 때 레이스로 되돌아온다는 것을 배웠습니다.
