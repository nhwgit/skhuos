...
3. **디스크 드라이버**: HDD 컨트롤러와 통신. LBA(Logical Block Addressing) 방식을 사용하여 지정된 섹터/실린더/헤드에서 블록 단위의 I/O 읽기/쓰기 구현. 섹터 준비·플러시 완료를 상태 폴링이 아닌 **디스크 인터럽트로 통지받는 인터럽트 구동 PIO**이며, 모든 대기는 PIT 스톱워치 기반 시간 타임아웃(500ms)으로 실패를 호출부에 반환.
...
7. **증상 없이 잠복해 있던 디스크 대기 코드 정리 — 횟수 기반 타임아웃과 뒤집힌 플래그**
   ...
   - **배운 점**: 증상이 없다는 것은 코드가 맞다는 증거가 아니라 환경이 관대하다는 뜻일 수 있습니다(5번의 연장). 특히 폴링이 인터럽트 경로를 가리는 구조에서는 플래그류 버그가 구조 전환(인터럽트 구동 I/O) 시점에야 터지므로, 전환 전에 의미 단위 점검으로 지뢰를 미리 제거해야 합니다. 그리고 대기·타임아웃은 횟수가 아니라 시간으로 정의해야 플랫폼이 바뀌어도 의미가 보존됩니다. 이 점검은 실제로 회수됐습니다 — 직후 디스크 I/O를 폴링에서 인터럽트 구동 PIO로 전환했을 때, 미리 제거해 둔 지뢰 덕분에 전환이 무증상으로 완료됐습니다.
