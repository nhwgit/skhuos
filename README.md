...
2. **빌드 단계 디스크 이미지 무한 증식 버그**
   ...
   - 이미지 도구의 입력을 명시적 경로로 고정하고, 빌드 산출물을 소스 트리 밖 별도 디렉터리(`build/`)로 분리해 자기참조 가능성을 구조적으로 차단했습니다. 이후 `ImageMaker` 자체에도 입력·출력 동일성 검사(stat 기반)를 추가해 코드 레벨 방어를 완성했습니다. 여기에 파일별 섹터 수 출력처럼 산출물 크기를 기대값과 대조하는 값싼 불변식 검증만 갖춰도 폭주형 버그를 조기에 잡을 수 있다는 교훈을 얻었습니다.
...
7. **부팅 방식 전환이 드러낸 드라이버의 잠복 레이스 버그**
   ...
   - 원인은 둘 다 ATA 프로토콜의 대기 규정 생략이었습니다. 1) 명령 발행 전 이전 명령의 BUSY 해제를 확인하지 않아 디바이스가 명령을 무시했고(커널은 오지 않을 DRQ를 영원히 대기) 2) 멀티섹터 쓰기에서 첫 DRQ만 확인하고 데이터를 연속으로 밀어넣어 디바이스가 직전 섹터를 플러시하는 동안 보낸 데이터가 유실됐습니다. 각각 BUSY 해제 대기와 "섹터마다 DRQ 대기" 구조로 수정했습니다.
