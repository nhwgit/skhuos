...
| 프로세스 | PCB · 라운드 로빈 선점형 스케줄러(5ms) · READY/BLOCKED 상태 모델(타이머 만료 큐 · idle 프로세스) · 대기 큐 기반 blocking 재진입 뮤텍스(FIFO 소유권 이양) |
| 특권 분리 | Ring 3 유저 모드(DPL3 세그먼트 · 페이지 U/S 보호) · `int 0x81` 시스템 콜(TSS RSP0 스택 전환 · 유저 포인터 검증) · 링3 예외 격리(프로세스만 종료) |
...
│   ├── syscall/       # 시스템 콜 디스패처 (기능 번호 분기 · 유저 포인터 검증)
│   ├── user/          # Ring 3 데모 프로그램 (커널이 유저 영역에 복사해 실행하는 자기완결 블롭)
...
- **"32비트 커널"이 아니라 `loader`.** 32비트 단계의 실제 책임은 페이지 테이블 준비와 64비트 커널의 복사·점프가 전부입니다. 관례적인 "Kernel32" 대신 실제 역할대로 로더라고 이름 붙였습니다.
...
3. 부팅 후 셸 프롬프트에서 `help`를 입력하면 사용 가능한 명령어 목록이 출력됩니다. 파일 시스템 명령어는 포맷·마운트가 선행되어야 합니다 — [터미널 셸 명령어](#7-터미널-셸-명령어-shell-commands) 참고.
...
typedef struct pcb {
    Node link;              // 준비/sleep/뮤텍스 대기 리스트 중 한 곳에만 속하므로 노드 공유 (id == pid)
...
- **동기화 (Synchronization)**: 대기 큐를 갖는 **blocking 재진입 뮤텍스**. 락 획득 실패 시 프로세스가 BLOCKED로 잠들어 CPU를 전혀 쓰지 않고, 해제 시 대기 큐 선두에게 **소유권을 직접 이양(direct handoff)**하므로 새 진입자가 락을 가로챌 수 없어 FIFO 공정성이 보장됩니다.
...
void acquireLock(Mutex * mutex) {
    ...
    if (mutex->pid == currentPid)          // 재진입: 카운트만 증가
        mutex->count++;
    else if (mutex->pid == -1) {           // 빈 락: 즉시 획득
...
### 5. 특권 분리 및 시스템 콜 (Ring 3 / System Call)

애플리케이션 코드를 커널(Ring 0)과 분리된 유저 모드(Ring 3)에서 실행하고, 커널 서비스는 시스템 콜을 통해서만 요청하게 하는 보호 구조입니다.

- **DPL3 디스크립터**: GDT에 유저 데이터(0x18)·코드(0x20) 세그먼트를 추가. 데이터→코드 순서는 향후 `syscall/sysret` 전환 시 STAR 규약과 호환되는 배치입니다.
- **페이지 U/S 보호**: 페이징 상위 레벨(PML4·PDPT·PD)은 U/S를 열어 두고 최종 권한은 PTE에서만 결정합니다. 유저 영역(코드·스택)의 PTE에만 U/S를 켜므로, 그 밖의 모든 메모리는 링3에서 접근하는 순간 페이지 폴트가 됩니다. 권한 변경 시에는 CR3 재적재로 TLB를 무효화합니다.
- **시스템 콜 진입점(`int 0x81`)**: DPL3 인터럽트 게이트 하나에 RAX 기능 번호 분기(화면 출력 · sleep · getpid · exit). 게이트는 IST 대신 **TSS RSP0(프로세스별 커널 스택)**로 진입하는데, sleep처럼 도중에 잠드는 시스템 콜 동안 발생하는 인터럽트(전 벡터가 IST 하나를 공유)가 시스템 콜 프레임을 덮지 않게 하기 위함입니다. RSP0은 컨텍스트 스위칭마다 다음 프로세스의 커널 스택으로 갱신되며, 유저 프로세스는 유저 스택(링3)과 커널 스택(시스템 콜)을 분리 보유합니다.
- **유저 포인터 검증**: 유저가 넘긴 포인터가 유저 영역 안의 NUL 종단 문자열인지 확인해, 커널 주소를 넘겨 커널 메모리를 출력시키는 요청을 차단합니다.
- **예외 격리**: 링3에서 발생한 예외는 예외 프레임의 CS RPL로 판별해, 시스템 정지(halt) 대신 **해당 프로세스만 종료**합니다 — 유저 프로그램의 오류가 시스템 전체를 멈추지 못합니다.

셸 명령 `ring3`가 데모입니다. 커널이 유저 영역에 복사한 자기완결 프로그램이 링3에서 자신의 CS(0x23, CPL=3)를 출력하고 시스템 콜로 sleep·화면 출력 후 종료하며, `ring3 attack`은 마지막에 커널 메모리를 직접 읽다가 페이지 폴트로 격리 종료됩니다 — [데모](#3-ring-3-특권-분리-ring3-attack) 참고.
...
| `ring3 [attack]` | Ring 3 유저 모드 데모 — attack 모드는 커널 메모리 접근이 차단·격리되는 것을 시연 |
...
### 3. Ring 3 특권 분리 (`ring3 attack`)

![Ring3 Demo](docs/images/ring3.png)

- 유저 프로그램이 링3에서 실행 중임을 스스로 증명합니다 — 출력된 `CS=0x23`은 유저 코드 세그먼트(0x20)에 RPL 3이 더해진 값이고, 하위 2비트가 곧 CPL=3입니다. 문장과 진행 표시(`.....`)는 전부 시스템 콜로 출력한 것입니다.
- 마지막에 커널 메모리(0x600000)를 직접 읽으려다 페이지 폴트가 발생하고, 커널은 시스템을 멈추는 대신 해당 프로세스만 종료합니다(하단 메시지 — RIP `0x30000CD`는 유저 코드 영역). 셸은 계속 동작합니다.
...
| 한계 | 현재 상태 | 확장 방향 |
| --- | --- | --- |
| 시스템 콜 최소 구현 | `int 0x81` 게이트 + 기능 4종, 유저 프로그램은 커널 이미지에 내장된 블롭 복사 | `syscall/sysret` 전환(GDT 배치는 STAR 규약 호환으로 준비됨), 파일 I/O 등 서비스 확대, 디스크에서 적재하는 실행 파일 로더 |
| 단일 주소 공간 | identity 매핑, 컨텍스트 스위칭 시 CR3 미교체 (유저/커널 구분은 페이지 U/S 비트뿐) | 프로세스별 페이지 테이블 + demand paging |
| 우선순위 없는 스케줄링 | 모든 프로세스가 동일 슬라이스(RR) | MLFQ 등 우선순위 스케줄링 (뮤텍스에는 우선순위 상속 동반 필요) |
