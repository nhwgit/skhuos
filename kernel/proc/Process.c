...
// 링3 프로세스가 시스템 콜(RSP0 스택 전환)로 진입할 커널 스택 — 실행 프로세스 교체마다 갱신
static void updateTSSRsp0(const PCB * next) {
	setTSSRsp0((QWORD)next->stackAddress + next->stackSize);
}
...
// [start, end) 페이지의 U/S 비트 설정 — 유저 모드(링3) 접근 허용 범위 지정
static void setPageUserAccess(QWORD start, QWORD end, bool allow) {
	PtEntry * page = (PtEntry *)PTABLE_BASE_ADDRESS;
	for(QWORD addr = start; addr < end; addr += 0x1000) {
		if(allow)
			page[addr >> 12].lower4Byte |= PAGE_LOWER4B_FLAGS_US;
		else
			page[addr >> 12].lower4Byte &= ~PAGE_LOWER4B_FLAGS_US;
	}
	flushTLB(); // TLB에 캐시된 이전 권한 무효화
}
...
void garbageProcessCollector(void) {
	...
	while(1) {
		...
		QWORD userStack = USER_STACK_BASE + USER_STACK_INTERVAL*(QWORD)pid;
		setPageUserAccess(userStack, userStack+USER_STACK_SIZE, FALSE); // 유저 프로세스였다면 회수(커널이면 원래 0)
		PtEntry * page = (PtEntry *)PTABLE_BASE_ADDRESS;
...
PCB * createUserProcess(QWORD entryPoint, QWORD arg) {
	bool preIf = setIf(FALSE); // 링3 레지스터 세팅이 끝나기 전 스케줄되지 않도록
	PCB * pcb = createProcess(entryPoint, arg);
	QWORD userStack = USER_STACK_BASE + USER_STACK_INTERVAL*(QWORD)pcb->link.id;
	setPageUserAccess(userStack, userStack+USER_STACK_SIZE, TRUE);
	pcb->context.reg[REG_CS] = USER_CODE_DESCRIPTOR_OFFSET | RPL_USER;
	pcb->context.reg[REG_SS] = USER_DATA_DESCRIPTOR_OFFSET | RPL_USER;
	pcb->context.reg[REG_DS] = USER_DATA_DESCRIPTOR_OFFSET | RPL_USER;
	pcb->context.reg[REG_ES] = USER_DATA_DESCRIPTOR_OFFSET | RPL_USER;
	pcb->context.reg[REG_FS] = USER_DATA_DESCRIPTOR_OFFSET | RPL_USER;
	pcb->context.reg[REG_GS] = USER_DATA_DESCRIPTOR_OFFSET | RPL_USER;
	pcb->context.reg[REG_RSP] = userStack + USER_STACK_SIZE;
	pcb->context.reg[REG_RBP] = userStack + USER_STACK_SIZE;
	// pid 슬롯 스택(stackAddress)은 그대로 남겨 시스템 콜 진입 시 RSP0 커널 스택으로 사용
	setIf(preIf);
	return pcb;
}

PCB * runUserProgram(const void * image, QWORD size, QWORD arg) {
	if(size > USER_CODE_SIZE)
		return NULL;
	bool preIf = setIf(FALSE);
	memcpy((void *)USER_CODE_ADDRESS, image, size);
	setPageUserAccess(USER_CODE_ADDRESS, USER_CODE_ADDRESS+USER_CODE_SIZE, TRUE);
	PCB * pcb = createUserProcess(USER_CODE_ADDRESS, arg);
	setIf(preIf);
	return pcb;
}
